...
| P0 | H617A segment read-back | Replies `AA A5 <group>` are decoded, but the request that elicits each group is not byte-pinned. | Capture the exact app TX request and all five RX groups. Do not infer the request from the reply selector. |
| P0 | H6199 video | The app always sends sound and softness, while `build_video_mode` can emit a shortened frame. | Correct the builder contract and byte-pin sound off with retained softness, minimum softness, and restoration. |
...
| P1 | H617A Workshop | Field locations are mapped, but several packed value spaces are incomplete. | Isolate Applied Area, distribution and direction combinations, movement flags and directions, priority values, and explicit layer reorder behaviour. |
| P1 | H617A rgbicv2 DIY | Transport and record grammar are known; from-scratch per-effect authoring is incomplete. | Isolate Brilliant and Sparkle ranges, remaining speed-mode arrays, direction and sub-style values, and colour parameters. |
| P1 | H617A adjustable scenes | Frozen catalogue configuration exists, but the current app's editor path was not found. | Establish one unambiguous editable scene path and complete one parameter A/B/A before expanding. |
