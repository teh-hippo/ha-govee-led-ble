...
def _codec_for(model: str, operation: str) -> WireProtocolCodec:
    ...
    if resolved is None or (codec := WIRE_PROTOCOL_CODECS.get(resolved)) is None:
        raise ValueError(f"{model} has no {operation} grammar codec")
    return codec
...
def _has_unknown_discriminator(parsed: Any, parser: str) -> bool:
    if isinstance(parsed, H6179StatusReply):
        return False
    if (
        isinstance(parsed, (StatusReply, H6199StatusReply))
        or parser.endswith("status_reply")
        or not hasattr(parsed, "opcode")
    ):
        return getattr(getattr(parsed, "domain", None), "name", None) is None
...
def _build_status_query(
    ...
    *,
    ...
    codec = WIRE_PROTOCOL_CODECS.get(grammar) if grammar is not None else None
    if codec is None:
        raise ValueError(f"{grammar} has no status-query grammar for {domain}")
    try:
        generated_domain = codec.query_domains[domain]
    except KeyError:
        raise ValueError(f"{grammar} has no {domain} status-query operation") from None
    ...
    if display_setting is not None:
        if root_type is not H6199StatusQuery:
            raise ValueError(f"{grammar} has no display-setting status-query operation")
        ...
        if root_type not in {StatusQuery, H6199StatusQuery}:
            raise ValueError(f"{grammar} has no segment status-query operation")
...
def build_mode_query(model: str = "H6179") -> bytes:
    return _build_status_query("mode", get_profile(model).command_grammar)
...
def build_scene_activation(
    ...
    *,
    ...
    if grammar == "H6179":
        return build_h6179_scene(scene_code)
    if grammar == "H6199":
...
def build_music_mode(
    ...
    profile = get_profile(model)
    if type(mode_id) is not int or music_slug_for(model, mode_id) not in profile.music_modes:
