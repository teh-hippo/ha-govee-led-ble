...
_CAMERA_COMMAND_ROOTS = {
    "H6099": H6099CommandWrite,
    "H6199": H6199CommandWrite,
}
_A3_EFFECT_ROOTS = {
    "H6099": H6199EffectUpload,
    "H6199": H6199EffectUpload,
}
...
def parse_a3_effect_envelope(envelope: bytes, model: str) -> Any:
    ...
    elif (root_type := _A3_EFFECT_ROOTS.get(model)) is None:
...
def _command_types(model: str) -> tuple[Any, Any, Any]:
    resolved = wire_model(model)
    if (root_type := _CAMERA_COMMAND_ROOTS.get(resolved or "")) is not None:
        return (
            root_type,
            root_type.PowerBody,
...
def _build_status_query(
    ...
    *,
    ...
    root_type = _STATUS_QUERY_ROOTS[resolved]
    root = root_type()
    root.header = b"\xaa"
...
def build_segment_query(group: int, model: str = "H617A") -> bytes:
    ...
    maximum = get_profile(model).segment_group_count
...
def build_h6199_scene(scene_code: int, music_code: int = 0) -> bytes:
    return build_camera_scene("H6199", scene_code, music_code)


def build_h6099_scene(scene_code: int) -> bytes:
    return build_camera_scene("H6099", scene_code)


def build_camera_scene(model: str, scene_code: int, music_code: int = 0) -> bytes:
    resolved = wire_model(model)
    root_type = _CAMERA_COMMAND_ROOTS.get(resolved or "")
    if root_type is None:
        raise ValueError(f"{model} has no generated camera-scene grammar")
    root = root_type()
    root.header = b"\x33"
    root.opcode = root_type.CommandOp.mode
    mode = _child(root_type.ModeBody, root)
    ...
    if root_type is H6199CommandWrite:
