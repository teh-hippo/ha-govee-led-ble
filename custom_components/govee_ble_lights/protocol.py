...
def build_packet(cmd_type: int, action: int, params: list[int]) -> bytes:
    """Build a 20-byte command packet with XOR checksum."""
...
def build_color_rgb(r: int, g: int, b: int) -> bytes:
    """Build segmented color command (all 15 segments)."""
...
def kelvin_to_rgb(kelvin: int) -> tuple[int, int, int]:
    """Convert Kelvin to RGB using Tanner Helland algorithm (1000-10000K)."""
    temp = max(1000, min(10000, kelvin)) / 100.0
    if temp <= 66:
        red = 255.0
        green = 99.4708025861 * math.log(temp) - 161.1195681661
    else:
        red = max(0.0, min(255.0, 329.698727446 * ((temp - 60) ** -0.1332047592)))
    ...
    if temp >= 66:
        ...
        blue = max(0.0, min(255.0, 138.5177312231 * math.log(temp - 10) - 305.0447927307))
    return int(red), int(green), int(blue)


def build_color_temp(kelvin: int) -> bytes:
    """Convert Kelvin to RGB and send as segmented color."""
    return build_color_rgb(*kelvin_to_rgb(kelvin))


def build_scene(scene_id: int) -> bytes:
    """Build scene command (little-endian code bytes)."""
...
def build_scene_multi(scene_param_b64: str, scene_code: int) -> list[bytes]:
    """Build multi-packet scene sequence from base64 scenceParam."""
    ...
    for i, chunk in enumerate(chunks):
        line_idx = 0xFF if i == len(chunks) - 1 else i
...
class ParsedColorModeResponse:
    """Parsed color-mode notify payload (0xAA 0x05)."""
...
def parse_color_mode_response(payload: bytes) -> ParsedColorModeResponse:
    ...
    if mode == ColorMode.VIDEO:
        return ParsedColorModeResponse(
            effect="video: game" if (len(payload) > 2 and bool(payload[2])) else "video: movie",
    ...
    if mode == ColorMode.MUSIC:
        ...
        return ParsedColorModeResponse(
            ...
            music_sensitivity=payload[2] if len(payload) > 2 else None,
            music_color=color,
        )
