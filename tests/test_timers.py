...
def test_sim_timer_queries_return_captured_disabled_defaults():
    sim = GoveeDeviceSim("H617A")
    assert proto.parse_timer_sleep(bytes(_query(sim, 0x11)[0])[2:-1]) == proto.ParsedSleepTimer(
        enabled=False,
        start_brightness=50,
        close_minutes=16,
        current_minutes=16,
    )
    assert proto.parse_timer_wakeup(bytes(_query(sim, 0x12)[0])[2:-1]) == proto.ParsedWakeUpTimer(
        enabled=False,
        end_brightness=100,
        hour=17,
        minute=1,
        repeat_days=frozenset(proto.Weekday),
        duration_minutes=29,
    )
...
async def test_mock_ble_sleep_timer_write_then_decode(mock_ble: MockBle):
    ...
    if not coord.profile.supports_timers:
        ...
        return
    await coord.async_set_sleep_timer(enabled=True, minutes=25)
    assert sim.sleep_timer == (1, 50, 25, 25)
...
async def test_mock_ble_wakeup_timer_write_then_decode(mock_ble: MockBle):
    ...
    if not coord.profile.supports_timers:
        ...
        return
    await coord.async_set_wakeup_timer(enabled=True, wake_time=dtime(6, 30))
    assert sim.wakeup_timer == (1, 100, 6, 30, 0, 29)
