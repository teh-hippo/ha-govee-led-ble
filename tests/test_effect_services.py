...
async def test_entity_action_applies_saved_effect_and_returns_deployment(
    ...
    reactive = SimpleNamespace(async_supersede_device=AsyncMock())
    entity = GoveeBLELight(
        mock_coordinator,
        config_entry_id="entry-a",
        effect_backend=backend,
        reactive_backend=cast(Any, reactive),
    )
    ...
    reactive.async_supersede_device.assert_awaited_once_with("entry-a")
