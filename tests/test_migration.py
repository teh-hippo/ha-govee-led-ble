...
async def test_migrate_current_entry_bumps_to_v9(hass: HomeAssistant):
...
async def test_migrate_v8_enables_the_h6125_rc3_categories(hass: HomeAssistant):
    entry = MockConfigEntry(
        domain=DOMAIN,
        unique_id=_ADDR,
        version=8,
        data={CONF_MODEL: "H6125"},
        options={CONF_EFFECT_CATEGORIES: []},
    )
    entry.add_to_hass(hass)
    ...
    assert entry.version == 9
    assert entry.options[CONF_EFFECT_CATEGORIES] == ["scenes", "effects", "multi_layered", "reactive"]
