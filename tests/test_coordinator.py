...
def test_h6099_camera_builders_use_exact_model_fields() -> None:
    video = parse_command(build_video("H6099", True, True, 42, True, 55), "H6099")
    white_balance = parse_command(build_h6099_white_balance(50), "H6099")
    black_border = parse_command(build_black_border(True), "H6099")

    assert video is not None
    assert video.body.detail.source.name == "game"
    assert video.body.detail.region.name == "all"
    assert video.body.detail.sound_type == b"\x02"
    assert white_balance is not None and white_balance.body.payload.progress == 50
    assert black_border is not None and black_border.body.payload.is_on == 1
