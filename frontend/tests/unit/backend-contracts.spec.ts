...
describe("focused effect-content mutations", () => {
  test("H6179 DIY and music content enforce the exact SKU contract", () => {
    ...
    expect(() =>
      decodeEffectContent({ ...music, sensitivity: 100 }),
    ).toThrow("music profile sensitivity must be an integer from 0 to 99");
    expect(() =>
      decodeEffectContent({ ...music, calm: false }),
    ).toThrow("H6179 music profiles do not support style or parameters");

    const paletteDiy = cloneObject(contentSamples.palette_diy);
    paletteDiy.model = "H6179";
    expect(() => decodeEffectContent(paletteDiy)).toThrow(
      "H6179 palette DIY content is unsupported",
    );
  });
