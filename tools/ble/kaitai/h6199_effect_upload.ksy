...
seq:
  - id: header
    contents: [0x01]
    doc: 'effect-body marker at body offset 0, captured as 0x01 throughout the committed corpus'
  - id: chunk_count
    type: u1
    doc: |
      how many 17-byte transport chunks the body occupies, at body offset 1.
  ...
  - id: kind
    ...
    enum: body_kind
    doc: |
      which shape the rest of the body takes, at body offset 2. Captured as
  ...
  - id: content
    ...
    doc: 'the effect definition from body offset 3, in the shape the kind byte selects'
enums:
  body_kind:
...
types:
  unmodelled_content:
    ...
    seq:
      - id: opaque_body
        size-eos: true
        doc: 'the entire body after the kind byte, captured from both shipped samples and their byte-identical repeats'
  scene_content:
    seq:
      - id: block_count
        type: u1
        doc: |
          how many blocks follow, at body offset 3. Captured from 1 through
      ...
      - id: blocks
        ...
        doc: 'the effect definition, as block_count length-prefixed blocks starting at body offset 4'
      - id: padding
        size-eos: true
        doc: |
          zero padding out to the transport chunk boundary. Captured as 1 to
  ...
  diy_content:
    ...
    seq:
      - id: family
        ...
        enum: effect_family
        doc: |
          the animation family, at body offset 3. Captured as 0 for the three
      ...
      - id: variant
        type: u1
        doc: |
          which style within the family, at body offset 4. NOT AN ORDINAL,
      ...
      - id: speed
        type: u1
        doc: |
          animation speed, at body offset 5. Captured as 0x32 and then 0x5c
      ...
      - id: palette_len
        ...
        valid:
          expr: _ % 3 == 0
        doc: |
          how many palette bytes follow, at body offset 6. Captured as 21
      ...
      - id: palette
        ...
        doc: 'the colours the effect cycles, from body offset 7, in the order the editor draws them'
      - id: padding
        size-eos: true
        doc: |
          zero padding out to the transport chunk boundary, captured as
  ...
  rgb:
    seq:
      - id: red
        type: u1
        doc: 'red channel; the editor default palette begins ff 00 00, which is the red swatch it draws first'
      - id: green
        type: u1
        doc: 'green channel; the fourth swatch is 00 ff 00 and the app draws it green'
      - id: blue
        type: u1
        doc: 'blue channel; the fifth swatch is 00 00 ff and the app draws it blue'
  brightness_block:
    doc: |
    ...
    seq:
      - id: scope_high
        size: 1
        doc: 'Brightness Scope upper handle, scaled to 0..255; moving 100% to 80% changed ff to cd alone'
      - id: scope_low
        size: 1
        doc: 'Brightness Scope lower handle, scaled to 0..255; moving 0% to 35% changed 00 to 5b alone'
      - id: order
        type: u1
        enum: brightness_order
        doc: 'Brightness Order: 0 Brightest-Darkest, 1 Brightest-Darkest-Brightest, 2 Darkest-Brightest, 3 Darkest-Brightest-Darkest'
      - id: change_speed
        size: 1
        doc: 'Brightness Changing Speed, scaled to 0..255; 50% is 80 and 93% is ed'
      - id: retention_brightest
        size: 1
        doc: 'Retention Time of the Brightest Light, written directly'
      - id: retention_darkest
        size: 1
        doc: 'Retention Time of the Darkest Light, written directly'
  block:
    doc: |
    ...
    seq:
      - id: len
        type: u1
        doc: 'the block length, not counting itself; captured between 26 and 47'
      - id: applied_area
        type: u1
        doc: |
          Applied Area at block offset 0. The high nibble is the width in
      ...
      - id: select_type
        ...
        doc: |
          Select Type at block offset 1: Segment 0, Select IC Continuously
          1, Select IC Randomly 2 and Customize Segment 3.
      - id: select_param_1
        type: u1
        doc: |
          first Select Type parameter at block offset 2. It is the random
          Maximum ICs, isolated 2 to 3; Customize Segment displayed 1 here.
      - id: select_param_2
        type: u1
        doc: |
          second Select Type parameter at block offset 3. It follows Number
          of Segment/IC and random Minimum IC; Customize Segment displayed 0 here.
      - id: layer_flags
        type: u1
        doc: |
          layer flags at block offset 4. Bit 0x02 selects Brightness
          Gradient rather than Unified, isolated by changing 0x12 to 0x10 alone. Other bits
          remain raw.
      - id: brightness_block_count
        type: u1
        doc: |
          number of six-byte Brightness sub-tabs. Adding tab 2 changed this
      ...
      - id: brightness_blocks
        ...
        doc: 'brightness_block_count six-byte Brightness sub-tabs'
      - id: distribution_direction
        type: u1
        doc: |
          packed Distribution Method and general Direction. Bit 0x80 means
      ...
      - id: colour_change_speed
        size: 1
        doc: 'Color Changing Speed, scaled to 0..255; 50% is 80 and 92% is ea'
      - id: retention_time
        size: 1
        doc: 'colour Retention Time, written directly'
      - id: colour_count
        type: u1
        doc: |
          number of RGB palette entries. Deleting one of two colours changed
          this byte from 2 to 1 and shortened the record by exactly three bytes.
      - id: palette
        type: rgb
        repeat: expr
        repeat-expr: colour_count
        doc: 'colour_count ordered RGB triplets from the Workshop palette'
      - id: selected_movement
        type: u1
        doc: |
          packed selected-area movement byte. Bit 0x10 enables movement,
      ...
      - id: selected_movement_interval
        size: 1
        doc: 'selected-area Moving Interval, written directly'
      - id: selected_movement_speed
        size: 1
        doc: 'selected-area Moving Speed, scaled to 0..255'
      - id: overall_movement
        size: 1
        doc: |
          packed Overall Moving Effect enable and direction, fixed four
          bytes from the record end
      - id: overall_movement_interval
        size: 1
        doc: 'overall Moving Interval, written directly'
      - id: overall_movement_speed
        size: 1
        doc: 'overall Moving Speed, scaled to 0..255'
      - id: layer_priority
        size: 1
        doc: 'Effect Layer Priority at the final record byte, 0 or levels 1 through 5'
      - id: excess
        size: 'len - 17 - brightness_block_count * 6 - colour_count * 3'
        doc: 'bytes not consumed by the complete record grammar; empty in every fixture'
    instances:
      applied_area_width_tenths:
        value: '(applied_area & 0xf0) >> 4'
        doc: 'Applied Area width in tenths'
      applied_area_start_tenths:
        value: 'applied_area & 0x0f'
        doc: 'Applied Area start in tenths'
      brightness_is_gradient:
        value: '(layer_flags & 0x02) != 0'
        doc: 'Brightness is Gradient when layer_flags bit 0x02 is set'
      brightness_scope_low:
        value: 'brightness_blocks[0].scope_low'
        doc: 'compatibility view of the first Brightness block lower scope'
      brightness_change_speed:
        value: 'brightness_blocks[0].change_speed'
        doc: 'compatibility view of the first Brightness block changing speed'
      retention_time_brightest:
        value: 'brightness_blocks[0].retention_brightest'
        doc: 'compatibility view of the first Brightness block brightest retention'
      retention_time_darkest:
        value: 'brightness_blocks[0].retention_darkest'
        doc: 'compatibility view of the first Brightness block darkest retention'
      distribution_method:
        value: 'distribution_direction & 0x7f'
        doc: 'Distribution Method value'
      direction_is_backward:
        value: '(distribution_direction & 0x80) != 0'
        doc: 'general Direction is Backward when bit 0x80 is set'
      selected_movement_enabled:
        value: '(selected_movement & 0x10) != 0'
        doc: 'selected-area movement is enabled when bit 0x10 is set'
      selected_enter_exit_enabled:
        value: '(selected_movement & 0x04) != 0'
        doc: 'Enter and Exit Effect is enabled when bit 0x04 is set'
      selected_direction:
        value: 'selected_movement & 0x03'
        doc: 'selected-area direction value'
